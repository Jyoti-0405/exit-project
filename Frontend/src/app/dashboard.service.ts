...
export class DashboardService {
  ...
  public putComment(review: Details, code: string): Observable<{}> {
    return this.http.put<{}>(this.url + '/insertDetail' + '/' + code, review);
  }
  
  //This method  is used to  get all comment list with their product id  by using  get request on api
  public getcommentList(code: string): Observable<Array<Details>> {
    return this.http.get<Array<Details>>(this.url + '/getAllDetails' + '/' + code);
  }
  ...
  public getQuestionsByUserName(userName: string): Observable<Array<Product>> {
    return this.http.get<Array<Product>>(this.url + '/getAllProducts' + '/' + userName);
  }
